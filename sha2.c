 ...
 * @Last modified time: 2018-03-23T09:10:33-04:00
 */
...
void sha1_Transform(const sha2_word32 *state_in, const sha2_word32 *data,
...
void sha1_Final(SHA1_CTX *context, sha2_byte digest[]) {
  ...
  if (usedspace == 0) {
    ...
    memset(context->buffer, 0, SHA1_SHORT_BLOCK_LENGTH);
    ...
    *context->buffer = 0x80;
    ...
    if (usedspace <= 56) {
      /* Set-up for the last transform: */
      memset(((uint8_t *)context->buffer) + usedspace, 0, 56 - usedspace);
    } else {
      if (usedspace < 64) {
        memset(((uint8_t *)context->buffer) + usedspace, 0, 64 - usedspace);
      }
...
void sha256_Transform(const sha2_word32 *state_in, const sha2_word32 *data,
                      sha2_word32 *state_out) {
...
void sha256_Final(SHA256_CTX *context, sha2_byte digest[]) {
  ...
  if (digest != (sha2_byte *)0) {
    ...
    if (usedspace > SHA256_SHORT_BLOCK_LENGTH) {
      memset(((uint8_t *)context->buffer) + usedspace, 0,
...
#define ROUND512_0_TO_15(a, b, c, d, e, f, g, h)                               \
  T1 =                                                                         \
      (h) + Sigma1_512(e) + Ch((e), (f), (g)) + K512[j] + (W512[j] = *data++); \
  (d) += T1;                                                                   \
...
void sha512_Transform(const sha2_word64 *state_in, const sha2_word64 *data,
